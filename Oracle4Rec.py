...
class FilterLayer(nn.Module):
    def __init__(self, args, device):
        ...
        self.out_dropout = nn.Dropout(args.hidden_dropout_prob)
        self.LayerNorm = LayerNorm(args.hidden_size, eps=1e-12).to(self.device)
        self.ratio = 0.75

    def forward(self, input_tensor):
        input_tensor = input_tensor.to(self.device)
        batch, seq_len, hidden = input_tensor.shape
        x = torch.fft.rfft(input_tensor, dim=1, norm='ortho').to(self.device)
...
class Intermediate(nn.Module):
    def __init__(self, args):
        ...
        self.device = args.device
        self.dense_1 = nn.Linear(args.hidden_size, args.hidden_size * 4).to(self.device)
        if isinstance(args.hidden_act, str):
            self.intermediate_act_fn = ACT2FN[args.hidden_act]
        else:
            self.intermediate_act_fn = args.hidden_act
        self.dense_2 = nn.Linear(4 * args.hidden_size, args.hidden_size).to(self.device)
        self.LayerNorm = LayerNorm(args.hidden_size, eps=1e-12).to(self.device)
        self.dropout = nn.Dropout(args.hidden_dropout_prob).to(self.device)
...
class MainModule(nn.Module):
    def __init__(self, args, device):
        ...
        self.filter_layer = nn.ModuleList([copy.deepcopy(filterlayer) for _ in range(1)])
...
class Encoder(nn.Module):
    ...
    def forward(self, input_ids, pos_ids, neg_ids, mode='past'):
        ...
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2).to(self.device)
...
class Oracle4Rec(nn.Module):
    def __init__(self, args, device='cuda:0'):
        ...
        self.transition = Transition(10+1, 0.2, device)
